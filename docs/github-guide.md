...
- 如果审查提了修改意见：
  ...
  3. **PR 会自动更新**，审查人能看到新改动
- 审查通过后，由 **李桂聿** 点绿色的 **Merge pull request** 合并进 `main`（仓库用普通 Merge 合并，保留完整的**分叉 / 合并历史**）
...
`main` 已开启分支保护，直接 push 会被拒绝。正常流程是：开分支 → 提交 → 建 PR → 在群里 @ 李桂聿 审查 → 由他合并。
