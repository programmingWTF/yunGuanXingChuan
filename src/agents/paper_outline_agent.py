"""
云观星传 - 论文大纲 Agent
职责：基于科学事实与校验结果，生成论文大纲（Outline，助研而非代写）
"""
import json
...
class PaperOutlineAgent(BaseAgent):
    ...
    def _build_user_prompt(self, input_data: Dict[str, Any]) -> str:
        ...
        prompt = f"""基于以下科学事实与校验结果，为研究者生成一份论文大纲（助研定位，只给框架与要点，不写正文）。

## 研究主题
{topic}

## 科学事实（三库之一）
{json.dumps(science_facts, ensure_ascii=False, indent=2)[:2500]}
...
5. evidence_sources 列出依据的三库证据"""
    ...
    def get_agent_info(self) -> Dict:
        return {
            ...
            "description": "论文大纲 Agent：生成结构完整的论文大纲（助研）",
            "input": "topic + 科学事实 + 校验结果",
