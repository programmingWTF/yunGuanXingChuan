...
export default function Inspiration() {
  const { projectId, status, rec, running, error, locked, exec, approve, confirmRerun, rerunConfirmEl } = useStageExec(1)
      ...
      {!projectId ? <NoProjectHint /> : (
        <div className="space-y-4">
          ...
          {directions.length === 0 && rec?.status === 'completed' && (
            <p className="text-[11px] text-slate-400">该阶段未生成方向卡片，可点击「重新运行」</p>
          )}
        </div>
      )}
      {rerunConfirmEl}
